# Reference from https://github.com/Asthestarsfalll/CrowdHuman.git
...
def crowdhuman2coco(args, odgt_path, json_path, data_path):
    ...
    bbox_id = 1
    categories = {}
    print(f"start convert {len(records)}")
    for image_id, image_dict in enumerate(records):
        ...
        for _, instance in enumerate(gt_box):
            ...
            if args.full:
                attr = instance['extra']
                if args.rm_occ and attr.get('ignore', False):
                    continue
            ...
            if args.head:
                ...
                if args.rm_hunsure and attr['unsure']:
                    continue
                if args.rm_hignore and attr.get('ignore', False):
                    continue
